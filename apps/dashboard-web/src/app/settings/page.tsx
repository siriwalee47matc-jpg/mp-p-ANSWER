...
const RISK_LEVEL_OPTIONS = [
  ...
  },
  {
    ...
    label: 'ตรวจจับและรายงานอัตโนมัติ',
    icon: '2',
  ...
  },
  {
    ...
    label: 'ปิดกั้นและคุ้มครองอัตโนมัติ',
    icon: '3',
...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const selectedLabel = RISK_LEVEL_OPTIONS.find((option) => option.value === selectedLevel)?.label || selectedLevel;
      setMessage({ type: 'success', text: `อัปเดตระดับการจัดการความเสี่ยงส่วนกลางเป็น "${selectedLabel}" เรียบร้อยแล้ว!` });
    } catch (err: any) {
      setMessage({ type: 'error', text: err.message });
    } finally {
      setSaving(false);
    ...
    <div>
      ...
      <main className="container">
        <div style={{ marginBottom: '2rem' }}>
          <h1 style={{ fontSize: '2rem', marginBottom: '0.25rem' }}>ตั้งค่าระดับการจัดการความเสี่ยงส่วนกลาง</h1>
          <p style={{ color: 'var(--text-muted)' }}>
            กำหนดระดับการจัดการความเสี่ยงส่วนกลางสำหรับระบบทั้งหมด (เฉพาะ ผู้ดูแลระบบ / ผู้ทบทวน)
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ maxWidth: '700px' }}>
            ...
            <div className="card" style={{ padding: '1.25rem', marginBottom: '1.5rem' }}>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '6px' }}>ระดับปัจจุบัน</div>
            ...
            <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
              ...
              {!canEdit && (
                <div style={{
                  ...
                  สิทธิ์เฉพาะ Admin หรือ Reviewer เท่านั้นที่สามารถเปลี่ยนแปลงค่านี้ได้
                </div>
              )}
            ...
            <div className="card" style={{ padding: '1.25rem', background: 'rgba(30,58,138,0.04)', border: '1px solid rgba(30,58,138,0.15)' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                ...
                <tbody>
                  <tr>
                    <td style={{ padding: '8px', fontWeight: 700, color: '#16a34a' }}>ตรวจสอบโดยเจ้าหน้าที่</td>
                    <td style={{ padding: '8px' }}>บันทึกข้อมูลเท่านั้น รอเจ้าหน้าที่</td>
                    <td style={{ padding: '8px', color: 'var(--text-muted)' }}>สภาวะปกติ / ทดสอบ</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '8px', fontWeight: 700, color: '#2563eb' }}>ตรวจจับอัตโนมัติ</td>
                    <td style={{ padding: '8px' }}>สแกนและแจ้งเตือนทางอีเมล</td>
                    <td style={{ padding: '8px', color: 'var(--text-muted)' }}>การทำงานประจำวัน</td>
                  </tr>
                  <tr>
                    <td style={{ padding: '8px', fontWeight: 700, color: '#dc2626' }}>ปิดกั้นอัตโนมัติ</td>
